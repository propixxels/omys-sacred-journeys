...
serve(async (req)=>{
  ...
  if (type === "verify-recaptcha") {
    ...
    if (!result.success && isSandbox) {
      console.warn("Bypassing reCAPTCHA in sandbox preview for verify endpoint:", {
        hostname,
        details: result
      });
      return new Response(JSON.stringify({
  ...
  if (type !== "newsletter" && recaptchaToken) {
    ...
    if (!result.success && isSandbox) {
      console.warn("Bypassing reCAPTCHA in sandbox preview:", {
        hostname,
        details: result
      });
    }
...
Message: ${message}
    ...
    if (!subject || !html) {
      return new Response(JSON.stringify({
        error: "Missing newsletter fields"
      }), {
        status: 400,
        headers: corsHeaders
      });
  ...
  try {
    ...
    return new Response(JSON.stringify({
      success: true,
      messageId: result.data?.email_id || result.email_id
    }), {
      status: 200,
      headers: corsHeaders
    });
